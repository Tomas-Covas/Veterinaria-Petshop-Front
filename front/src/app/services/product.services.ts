...
const getCategoryImage = (
  ...
  if (
    productName.includes("alimento") ||
    productName.includes("balanceado") ||
    productName.includes("comida") ||
    productName.includes("chow") ||
    productName.includes("whiskas") ||
    productName.includes("pedigree") ||
    productName.includes("royal canin")
  ) {
    return alimento;
  }
  if (
    productName.includes("collar") ||
    productName.includes("arnés") ||
    productName.includes("correa")
  ) {
    return collar;
  }
  if (
    productName.includes("juguete") ||
    productName.includes("pelota") ||
    productName.includes("hueso") ||
    productName.includes("cuerda") ||
    productName.includes("peluche") ||
    productName.includes("ratón")
  ) {
    return juguete;
  }
  if (
    productName.includes("cama") ||
    productName.includes("cucha") ||
    productName.includes("rascador")
  ) {
    return cucha;
  }
  if (
    productName.includes("shampoo") ||
    productName.includes("acondicionador") ||
    productName.includes("limpiador") ||
    productName.includes("toallitas") ||
    productName.includes("cepillo")
  ) {
    return shampoo;
  }
  if (
    productName.includes("transportadora") ||
    productName.includes("trasladador")
  ) {
    return trasladador;
  }
  ...
  switch (categoryId) {
    ...
    case 6:
      return trasladador;
    default:
      return alimento; // Default genérico
  }
};
...
export const getAllProducts = async (): Promise<IProduct[]> => {
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `getAllProducts: fetch failed ${res.status} ${
          res.statusText
    ...
    if (!contentType.includes("application/json")) {
      const text = await res.text();
      console.error(
        `getAllProducts: expected JSON but got ${contentType} — ${text.substring(
          0,
          200
        )}`
      );
...
export const getProductById = async (id: string): Promise<IProduct> => {
  ...
  const product = allProducts.find(
    (product) => product.id.toString() === id || product.id === Number(id)
  );
  if (!product) {
